...
##---------------------------------------------------------------------------##
  ...
  if calc == 'CCD':
    ...
    R_ijab = amplitude.doubles(I_oo,I_vv,Ivvvv,Ioooo,Iovvo,Iovvo_2,Iovov,Iovov_2,t2,t2)
    R_ijab = amplitude.symmetrize(R_ijab)
    oldt2 = t2.copy()   #cp.deepcopy(t2)
    eps_t, t2 = amplitude.update_t2(R_ijab,t2)
    if inp.diis == True:     #maybe try using if (a and b)
      if x+1>max_diis:
        # Limit size of DIIS vector
        if (len(diis_vals_t2) > max_diis):
          del diis_vals_t2[0]
...
##---------------------------------------------------------------------------##
  ...
  if calc == 'CCSD':
    ...
    #file2 = open("CCSD_energy_0.34_str_geom.dat","a")
    #file2.write('%s' %(x+1) + "\t"+ '%s' %(E_hf+E_ccd) + "\n")
    #file2.close 
    val = convergence(E_ccd,E_old,eps_t)
    if val == True :
      break
...
##---------------------------------------------------------------------------##
  ...
  if calc == 'ICCSD':
    ...
    eps_t, t1, t2 = cc_update.update_t1t2(R_ia,R_ijab,t1,t2)
    #file1 = open("iCCSDn_t2_amp_0.28_all_denom.dat","a")
    #file1.write('%s' %(x) + "\t"+ '%s' %t2[4,4,0,0] + "\t"+ '%s' %t2[3,3,0,0] + "\n")
    #file1.close 
    ...
    ###### ---- try writing files with df or np.save
    ####Save with: np.save('iCCSDn_t2_amp'+str(x), t2[0:occ, 0:occ, 0:nao-occ,0:nao-occ])
    ####Open with np.load('iCCSDn_t2_amp'+str(x)+'.npy')

    '''if (x == 4000):
      file1 = open("iCCSDn_t2_amp.dat","a")
      for i in range(0,occ):
        for j in range(0,occ):
          for a in range(occ,nao):
            for b in range(occ,nao):
              file1.write('%s' %(x) + "\t"+ '%s' %(t2[i,j,a-occ,b-occ]) + "\n")
      file1.close 

    if (x == 4001):
      file2 = open("iCCSDn_t2_amp2.dat","a")
      for i in range(0,occ):
        for j in range(0,occ):
          for a in range(occ,nao):
            for b in range(occ,nao):
              file2.write('%s' %(x) + "\t"+ '%s' %(t2[i,j,a-occ,b-occ]) + "\n")
      file2.close 
    
    if (x == 1002):
      file3 = open("iCCSDn_t2_amp3.dat","a")
      for i in range(0,occ):
        for j in range(0,occ):
          for a in range(occ,nao):
            for b in range(occ,nao):
              file3.write('%s' %(x) + "\t"+ '%s' %(t2[i,j,a-occ,b-occ]) + "\n")
      file3.close 

    if (x == 1003):
      file4 = open("iCCSDn_t2_amp4.dat","a")
      for i in range(0,occ):
        for j in range(0,occ):
          for a in range(occ,nao):
            for b in range(occ,nao):
              file4.write('%s' %(x) + "\t"+ '%s' %(t2[i,j,a-occ,b-occ]) + "\n")
      file4.close 

    if (x == 1004):
      file5 = open("iCCSDn_t2_amp5.dat","a")
      for i in range(0,occ):
        for j in range(0,occ):
          for a in range(occ,nao):
            for b in range(occ,nao):
              file5.write('%s' %(x) + "\t"+ '%s' %(t2[i,j,a-occ,b-occ]) + "\n")
      file5.close 
    '''
    #resfile[x]=("%8f" %(E_hf+E_ccd))
    #file1 = open("iCCSDn_energy_"+str(eta)+"_all_denom.dat","a")
    #file1.write('%s' %(x+1) + "\t"+ '%s' %(E_hf+E_ccd)+"\n")
    #file1.write('%s' %(E_hf+E_ccd)+ "\n")
    #file1.close 
...
#df=pd.DataFrame(data = resfile.astype(float))
#df.to_csv('iCCSDn_energy_'+str(eta)+'_all.csv', sep = ' ', header = False, index = False)
#result=np.array(resfile[-512:])
