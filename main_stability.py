...
##---------------------------------------------------------------------------------------------------------##
      ...
      dict_Y_ia[r,iroot] += np.einsum('ca,ic->ia',int_one_body,t1)    #could try np.multiply
      ...
      dict_Y_ia[r,iroot] += -np.einsum('ik,ka->ia',int_one_body2,t1)     #np.multiply
      int_one_body2 = 2.0*np.einsum('dclk,ld,ic->ik',twoelecint_mo[occ:nao,occ:nao,:occ,:occ],dict_t1[r,iroot],t1)
      dict_Y_ia[r,iroot] += -np.einsum('ik,ka->ia',int_one_body2,t1)     #np.multiply
      
      dict_Y_ia[r,iroot] = np.divide(dict_Y_ia[r,iroot],(D1+eta))
      dict_Y_ia[r,iroot] += dict_t1[r,iroot]
...
##----------------------------------------------------------------------------------------------------------##
      ...
      dict_Y_ijab[r,iroot] = np.divide(dict_Y_ijab[r,iroot],(D2+eta))
      dict_Y_ijab[r,iroot] += dict_t2[r,iroot]
...
##----------------------------------------------------------------------------------------------------------## 
      ...
      if (tiCCSD):
        ...
        dict_Y_iuab[r,iroot] = np.divide(dict_Y_iuab[r,iroot],(Dv+eta))
        dict_Y_iuab[r,iroot] += dict_Sv[r,iroot]
...
##----------------------------------------------------------------------------------------------------------##
        ...
        dict_Y_ijav[r,iroot] = np.divide(dict_Y_ijav[r,iroot],(Do+eta))
        dict_Y_ijav[r,iroot] += dict_So[r,iroot]
...
##----------------------------------------------------------------------------------##
    ...
    for iroot in range(0,nroot):
      for jroot in range(0,nroot):
        ...
        #B_Y_ia[loc1,loc2] = 2.0*np.einsum('ia,ia',np.divide(dict_t1[r,iroot],(D1+eta)),dict_Y_ia[r,jroot])
        #B_Y_ia[loc1,loc2] += 2.0*np.einsum('ia,ia',dict_t1[r,iroot],dict_t1[r,iroot])
        #B_Y_ijab[loc1,loc2] = 2.0*np.einsum('ijab,ijab',np.divide(dict_t2[r,iroot],(D2+2*eta)),dict_Y_ijab[r,jroot])-np.einsum('ijba,ijab',np.divide(dict_t2[r,iroot],(D2+2*eta)),dict_Y_ijab[r,jroot])
        #B_Y_ijab[loc1,loc2] += 2.0*np.einsum('ijab,ijab',dict_t2[r,iroot],dict_t2[r,iroot])-np.einsum('ijba,ijab',dict_t2[r,iroot],dict_t2[r,iroot]) 
...
##--------------------------------------------------------------------------------------##
    ...
    if(r == 0):
      ...
      for iroot in range(0,nroot):
        ind_max_wtotal = np.argmax(w_total)                          #to calculate the minimum eigenvalue location from the entire spectrum
        dict_coeff_total[iroot] = vects_total[:,ind_max_wtotal].real  #to calculate the coeff matrix from eigen function corresponding to lowest eigen value
        dict_v_nth[iroot] = dict_coeff_total[iroot]
        w.append(w_total[ind_max_wtotal])
        w_total[ind_max_wtotal] = 0.0000001 
...
##---------------------------------------------------------------------------##
      ...
      print 'Eigenvalue for IROOT :',iroot+1, '  IS:  ', abs(w[iroot]) 
      print ("             ------------------------")
    ...
    if (sum(count)== nroot):
      for iroot in range(0,nroot):
        print "!!!!!!!!!!CONVERGED!!!!!!!!!!!!"
        print 'HIGHEST EIGENVALUE for sym', isym, 'iroot', iroot+1, ' :', abs(w[iroot])
